...
/* SIDEBAR */
.sb{width:258px;min-width:258px;background:var(--side);border-right:1px solid var(--brd);display:flex;flex-direction:column;overflow:hidden;transition:width .22s ease,min-width .22s ease}
.sb-head{padding:14px 14px 13px;border-bottom:1px solid var(--brd);flex-shrink:0;display:flex;align-items:flex-start;justify-content:space-between;gap:6px}
.sb-head-content{flex:1;min-width:0;overflow:hidden}
.sb-toggle{width:24px;height:24px;border-radius:5px;border:1px solid var(--brd);background:var(--card);color:var(--txt2);font-size:.75rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer;transition:all .15s;margin-top:2px}
.sb-toggle:hover{border-color:var(--acc);color:var(--acc)}
/* Collapsed sidebar (desktop) */
.sb.collapsed{width:52px;min-width:52px}
.sb.collapsed .sb-head-content,.sb.collapsed .sb-tagline,.sb.collapsed .sb-sub,
.sb.collapsed .si-txt,.sb.collapsed .grp,.sb.collapsed .si-child-grp,
.sb.collapsed .hist-lbl{display:none!important}
.sb.collapsed .si{padding:8px 0;justify-content:center;border-left:none;border-radius:6px;margin:1px 6px}
.sb.collapsed .si.active{border-left:none}
.sb.collapsed .si-ico{width:22px;height:22px}
.sb.collapsed .hist-btn{margin:4px 6px;padding:8px 0;justify-content:center;font-size:1rem}
.sb.collapsed #user-bar{flex-direction:column;gap:4px;padding:6px 4px;align-items:center}
.sb.collapsed #user-bar span{display:none}
/* Backdrop + mobile */
.sb-backdrop{display:none;position:fixed;inset:0;z-index:199;background:rgba(0,0,0,.45);backdrop-filter:blur(1px)}
.sb-backdrop.show{display:block}
.mob-ham{display:none;width:30px;height:30px;border-radius:5px;border:1px solid var(--brd);background:var(--card);color:var(--txt2);font-size:.85rem;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer;transition:all .15s}
.mob-ham:hover{border-color:var(--acc);color:var(--acc)}
@media(max-width:768px){
  .sb{position:fixed;left:0;top:0;height:100%;z-index:200;width:258px!important;min-width:258px!important;transform:translateX(-100%);transition:transform .25s ease}
  .sb.mob-open{transform:translateX(0)}
  .sb.collapsed{width:258px!important;min-width:258px!important;transform:translateX(-100%)}
  .sb.collapsed .sb-head-content,.sb.collapsed .si-txt,.sb.collapsed .grp,.sb.collapsed .hist-lbl{display:revert!important}
  .sb.collapsed .si{padding:8px 13px;justify-content:flex-start;border-left:2px solid transparent;border-radius:0;margin:0}
  .sb.collapsed .hist-btn{margin:10px 12px 12px;padding:7px 10px;justify-content:flex-start;font-size:.76rem}
...
<div class="sb-backdrop" id="sb-backdrop" onclick="_sbClose()"></div>
<div class="layout" style="display:none">
  <aside class="sb" id="main-sb">
    <div class="sb-head">
      <div class="sb-head-content">
      <div class="sb-logo">
      ...
      <button class="sb-toggle" id="sb-toggle" onclick="_sbToggle()" title="Contraer/expandir">&#8249;</button>
    </div>
    ...
    <button class="hist-btn" id="dashboard-btn" onclick="showDashboard()">&#128200;<span class="hist-lbl">&nbsp; Dashboard</span></button>
    <button class="hist-btn" id="codigos-btn" onclick="showCodigos()">&#128214;<span class="hist-lbl">&nbsp; C&#xF3;digos de Retorno</span></button>
    <button class="hist-btn" id="hist-btn" onclick="showHistorial()">&#128203;<span class="hist-lbl">&nbsp; Historial de Pruebas</span></button>
    <button class="hist-btn" id="agenda-btn" onclick="showAgenda()">&#128197;<span class="hist-lbl">&nbsp; Agenda</span></button>
    <button class="hist-btn" id="settings-btn" onclick="showSettings()">&#9881;<span class="hist-lbl">&nbsp; Settings</span></button>
  </aside>
  <main class="main">
    <div class="topbar">
      <button class="mob-ham" id="mob-ham" onclick="_sbOpen()" title="Menu">&#9776;</button>
...
(function(){
  try{
    ...
    var sb=document.getElementById('main-sb');
    var btn=document.getElementById('sb-toggle');
    if(c==='1'&&sb){sb.classList.add('collapsed');if(btn)btn.textContent='›';}
  }catch(e){}
})();
