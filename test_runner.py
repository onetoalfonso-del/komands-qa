...
async def api_run(suite_id: str, request: Request):
    ...
    if _activ_runs is not None:
        async def sse_activ():
            ...
            yield f"data: {json.dumps({'e':'line','t':f'Suite Activación — {len(_activ_runs)} TCs · cadena completa 6 pasos · delays: asig 1min · ia 1min · activ 3min'})}\n\n"
            yield f"data: {json.dumps({'e':'line','t':'━'*55})}\n\n"
